...
\dropcap{A}nimal behavior-inspired algorithms enable efficient, adaptive solutions to complex problems by mimicking the coordinated actions of animals, swarm behaviours, such as flocking or schooling. This project explores the Sheepdog-Driven Algorithm for Sheep Herd Transport by Liu et al., 2021 \cite{liu2021}, that moves beyond traditional swarm systems, where collective behavior emerges naturally within a group, to address a reverse problem: how a single agent (a "sheepdog") can control and direct a swarm (a "herd of sheep") from one location to another.
 The objective is to replicate and evaluate this sheepdog-inspired approach, focusing on how a single controlling agent can dynamically influence the herd to accomplish transport tasks.
...
% Similarly, Lien and Pratt (2009) explored interactive herding methods, demonstrating techniques for a single agent to dynamically direct a group while maintaining cohesion and navigating obstacles. Their work provides valuable insights for real-time control applications in both simulation and robotics. 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.6\linewidth]{fig/step17.PNG}
	\caption{The starting phase of herding}
	\label{fig:start}
\end{figure}
